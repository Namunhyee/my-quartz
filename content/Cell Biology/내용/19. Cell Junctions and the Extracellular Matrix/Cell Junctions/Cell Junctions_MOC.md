---
dg-publish: true
---
# Cell Junctions_MOC

## 동물 세포가 결합하는 두 가지 방식

동물 조직에서 세포들이 함께 유지되는 방식은 크게 두 가지로 나뉜다.

![[Cell Biology/출처/md converted/Chapter 19/_page_1_Figure_2.jpeg]]
*Figure 19-1. 두 가지 주요 조직 유형에서 동물 세포가 결합되는 방식.*

**Connective tissue** (예: 뼈, 힘줄)에서는 extracellular matrix가 주된 stress-bearing 구성요소이다. 세포들은 matrix 안에 드문드문 분포하며, *cell–matrix junction*을 통해 matrix에 부착한다. 이 결합은 세포가 matrix 안을 이동하고, 그 역학적 특성을 감지하도록 해준다.

**Epithelial tissue** (예: 장 내벽, 피부 표피)에서는 세포들이 **epithelium**이라는 시트 형태로 밀착 결합한다. Extracellular matrix는 주로 시트 아래의 얇은 층인 **basal lamina** (= basement membrane)로만 존재한다. 세포 내부의 cytoskeleton이 주된 stress-bearing 구성요소이며, *cell–cell junction*을 통해 인접 세포의 cytoskeleton과 연결된다.

---

## Cell Junction의 분류 (Figure 19-2 기준)

![[Cell Biology/출처/md converted/Chapter 19/_page_1_Figure_6.jpeg]]
*Figure 19-2. 척추동물 epithelial cell에서 발견되는 각종 cell junction의 요약. 소장 상피세포를 기반으로 한 도식.*

소장의 simple columnar epithelium을 기준으로, 세포 측면(lateral surface)에는 다음 junction들이 존재한다:

- **Anchoring junctions** — 세포의 cytoskeleton을 인접 세포 또는 ECM에 고정하여 기계적 강도를 제공한다.
  - [[Cell-Cell junction_MOC|Cell–cell]]: **[[Adheres junction_MOC|Adherens junction]]** ([[100_Actin filaments_MOC|actin]] 연결), **[[Desmosome_MOC|Desmosome]]** ([[200_Intermediate filaments_MOC|intermediate filament]] 연결)
  - [[Cell-Matrix Junctions_MOC|Cell–matrix]]: **Actin-linked cell–matrix junction**, **[[hemidesmosomes|Hemidesmosome]]** ([[200_Intermediate filaments_MOC|intermediate filament]] 연결)
- **[[Tight junction_MOC|Tight junction]]** — apical 쪽에서 세포 사이 공간을 봉합하여 분자의 paracellular 이동을 차단
- **[[Gap junction_MOC|Gap junction]]** — 인접 세포 cytoplasm을 직접 연결하는 채널
- **[[Cell-Matrix Junctions_MOC|Cell-Matrix anchoring Junctions]]** — [[Basal Lamina_MOC|basal lamina]] 혹은 [[Extracellular Matrix_MOC|ECM]]과 [[100_Actin filaments_MOC|actin filaments]], [[Keratin filaments]] 등을 연결
Apical 영역에서 tight junction, adherens junction, desmosome이 순서대로 배열된 구조를 **junctional complex**라고 한다.

---

## Transmembrane Adhesion Proteins의 역할 (Figure 19-3)

![[Cell Biology/출처/md converted/Chapter 19/_page_2_Picture_2.jpeg]]
*Figure 19-3. Transmembrane adhesion protein은 cytoskeleton을 세포 외부 구조와 연결한다. 외부 연결 대상에 따라 cell–cell junction 또는 cell–matrix junction으로 나뉜다. 내부 연결은 intracellular adaptor protein을 통해 간접적으로 cytoskeleton에 이루어진다.*

모든 anchoring junction은 plasma membrane을 가로지르는 **transmembrane adhesion protein**에 의존한다. 이 단백질은 한쪽 끝이 세포 내 cytoskeleton에, 다른 쪽 끝이 세포 외부 구조에 연결된다. 이들은 두 superfamily로 나뉜다.

| Junction                          | Transmembrane adhesion protein | Extracellular ligand          | Cytoskeletal attachment | Intracellular adaptors                        |
| --------------------------------- | ------------------------------ | ----------------------------- | ----------------------- | --------------------------------------------- |
| Adherens junction                 | Classical cadherins            | Cadherin (인접세포)               | Actin                   | α-Catenin, β-catenin, p120-catenin, vinculin  |
| Desmosome                         | Desmoglein, desmocollin        | Desmoglein/desmocollin (인접세포) | Intermediate filaments  | Plakoglobin, plakophilin, desmoplakin         |
| Actin-linked cell–matrix junction | Integrin                       | ECM proteins                  | Actin                   | Talin, kindlin, [[vinculin]], paxillin, FAK 등 |
| Hemidesmosome                     | α6β4 integrin, collagen XVII   | ECM proteins                  | Intermediate filaments  | Plectin, BP230                                |


*Table 19-1 기반.*

**Cadherin superfamily** → 주로 cell–cell attachment를 담당  
**Integrin superfamily** → 주로 cell–matrix attachment를 담당

---

## 하위 항목

- [[Homophilic vs heterophilic binding]]
- [[Cell-Cell junction_MOC]]
- [[Cell-Cell junction_MOC]]