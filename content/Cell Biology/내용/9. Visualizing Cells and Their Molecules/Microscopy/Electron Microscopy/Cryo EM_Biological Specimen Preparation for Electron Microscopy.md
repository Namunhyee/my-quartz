...
### (1) 고에너지 전자빔
- 전자빔은 매우 높은 에너지를 가진다.
- 생체 분자(단백질, 지질 등)가 손상될 수 있다.
### (2) 진공 환경
- Electron microscopy는 고진공 상태에서 수행된다.
- 살아있는 세포는 관찰할 수 없다.
### (3) 투과 조건
- Transmission Electron Microscopy (TEM)에서는 전자빔이 시료를 통과해야 한다.
- 따라서 시료를 매우 얇게 절단해야 한다 (ultrathin section).
## 2. Conventional TEM 시료 준비 과정
### (1) 고정 (Fixation)
목적: 단백질과 세포 구조를 안정화하여 원래 구조를 최대한 보존
- **Glutaraldehyde**: 단백질 고정
- **Osmium tetroxide (OsO4)**: 지질 및 세포막 구조 보존, 전자 밀도 증가
→ 세포막과 세포 내 구조를 안정적으로 유지
### (2) 탈수 (Dehydration)
- 시료 내 물을 유기용매로 대체
- 진공 환경에서 물은 유지될 수 없기 때문
### (3) Resin infiltration 및 Embedding
- 레진을 침투시켜 블록 형태로 굳힘
- 단단한 상태로 만들어 절단이 가능하도록 함
### (4) 초박절편 (Ultramicrotomy)
...
- <mark style="background:#fff88f">물을 얼음 결정이 아닌 vitreous ice 형태로 유지</mark>
- 화학 고정 없이 구조 보존
### Cryo-sectioning
- 두꺼운 세포를 <mark style="background:#fff88f">집속 이온 빔(FIB)으로 얇게 가공</mark>
- 판 모양(lamella)으로 만든 후 관찰
