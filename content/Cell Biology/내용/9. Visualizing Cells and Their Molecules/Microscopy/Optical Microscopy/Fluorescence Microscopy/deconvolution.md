...
### [[STED(Stimulated Emission Depletion)#Point Spread Function의 이해|Point Spread Function(PSF)]]의 이해
...
2. 이미지에 대한 blurring의 효과가 무엇이었을지 결정
	- <mark style="background:#fff88f">추가 수업내용</mark>) 컴퓨터가 계산해서 만든 가상의 뿌연 이미지와 실제로 현미경으로 관찰된 진짜 뿌연 이미지를 비교
		차이가 크면 진짜 모습이 아니다
		차이가 작으면 진짜 모습이다
- 흐려진 이미지에서 psf 신호 최소화, 노이즈 제거->이미지 추론
1. 동등한 "deblurring" (deconvolution) 적용
2. 흐릿한 three-dimensional image를 일련의 깨끗한 optical sections으로 변환
...
deconvolution은 CT와 비슷해보이지만 찍어놓은 사진으로 선명하게 만드는 것일 뿐 3d 재구성을 한다는 의미는 아니다.
