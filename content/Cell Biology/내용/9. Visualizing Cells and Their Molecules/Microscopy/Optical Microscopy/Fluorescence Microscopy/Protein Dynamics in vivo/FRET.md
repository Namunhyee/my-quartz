...
- Fluorochrome 선택 기준:
  ...
  - <mark style="background:#fff88f">두 스펙트럼이 overlap(겹침)해야 함</mark>[^기출_중간_2025_31]

### Energy Transfer 메커니즘

**조건**:
- 두 단백질이 상호작용하여 fluorochromes가 <mark style="background:#fff88f">매우 가까운 근접 거리(약 5 nm 이내</mark>)로 접근[^기출_중간_2025_31]
...
- 세포 손상 최소화
### 2. 높은 공간 해상도
- 1-5 nm 범위의 분자 간 거리 측정[^기출_중간_2025_31]
- FRET 효율은 거리(r)의 6제곱에 반비례(1/r⁶)하므로, donor와 acceptor가 멀어질수록 FRET 효율은 감소한다.[^기출_중간_2025_31]
- 일반 광학 현미경([[diffraction limit|회절 한계]] 약 200 nm)보다 훨씬 높은 해상도
### 3. 동적 정보
- 단백질 상호작용의 시작과 종료 추적[^기출_중간_2025_31]
- 단백질 간 상호작용, 분자간 거리 측정, 세포 신호전달 등 다양한 목적에 사용되며 3차 구조 직접 관찰에만 한정되지 않는다.[^기출_중간_2025_31]
- 상호작용의 강도 변화 측정
### 4. 정량적 분석
- FRET 효율 측정을 통한 정량적 데이터 획득
- FRET은 비방사적(nonradiative) 공명 에너지 전달을 이용하며, 광표백(photobleaching)을 이용하는 FRAP과는 근본적으로 다른 원리이다.[^기출_중간_2025_31]
...
[^기출_중간_2025_31]: [[Cell Biology/기출 DB/중간고사/2025년/mid_2025_31번|2025 중간 31번]] — FRET는 두 분자가 ~5nm 이내일 때 발생한다는 선지 ②가 정답. 선지 ①(동일 파장 방출 시 발생) 오답 근거: donor emission과 acceptor absorption이 overlap해야 함. 선지 ③(거리 증가 시 효율 증가) 오답 근거: FRET 효율은 거리에 반비례(1-5nm 범위 측정). 선지 ④⑤ 오답 근거: FRET는 단백질 상호작용/분자간 거리 측정에 광범위하게 사용되며, photobleaching(FRAP)과 무관.