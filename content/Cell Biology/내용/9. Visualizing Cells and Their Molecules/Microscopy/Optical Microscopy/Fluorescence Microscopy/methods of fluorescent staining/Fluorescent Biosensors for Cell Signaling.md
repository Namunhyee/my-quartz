...
- **색상 코딩**: <mark style="background:#ff4d4f">빨간색(가장 높은 농도) ~ 파란색(가장 낮은 농도)</mark>(교과서 내용)
	- 강의노트에는 <mark style="background:#fff88f">빨간색: 신호 약함/ 노란색: 신호 강함</mark> 라고 되어 있음
**관찰 결과**: 가장 높은 Ca²⁺ levels는 수천 개의 dendritic branches에 존재
==모든 indicator가 Ca2+ 에 결합하는 것이 아니라 결합하는 것과 안 한 것이 나뉘어 농도에 의해 색 차이가 형성됨.== 이라고 강의노트에 쓰여있지만
==수업시간에는 Ca이온의 결합 여부에 따라 구조가 바뀌어 방출하는 빛 파장이 달라진다고 하심.==


> [!NOTE] 질의응답 official
> 핵심은 Ca²⁺ fluorescent indicator는 Ca²⁺ 결합 여부에 따라 형광 세기 또는 스펙 트럼 특성이 달라지는 것을 의미합니다. 세포 내 Ca²⁺ 농도에 따라 bound/unbound dye 비율이 변하며, 이 를 pseudocolor로 표시하면 색 차이로 농도 변화를 시각화할 때 우리가 어떤 색으로 표시 하는지에 따라 차이가 있는 것 같습니다. 그러나 혼란의 여지가 있으니 시험 문제에는 출제하지 않겠습니다.

## Figure 9-22: Genetically Encoded Fluorescent Biosensors

![[Cell Biology/출처/md converted/Chapter 9/_page_14_Figure_2.jpeg]]
