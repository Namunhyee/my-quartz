...
- 결과: ER에서 misfolding → **[[ER-associated degradation|ERAD]]**에 의해 분해 → 세포막에 CFTR 없음
	- 분해되지 않으면 작동할 수 있는 단백질임. 즉 trivial한 돌연변이였던 것.
...
- PEX protein receptor에 의해 peroxisome 으로 운반.
- 일부 단백질은 ER을 거쳐 peroxisome 막으로 이동
...
- 동물: 미토콘드리아 + peroxisome 분담. peroxisome은 주로 VLCFA 담당당
...
| 복합체       | 위치             | 역할                                                                                                |
| --------- | -------------- | ------------------------------------------------------------------------------------------------- |
| **TOM**   | Outer membrane | 모든 핵 암호화 단백질의 첫 번째 관문; cytosol에서 signal sequence 인식 → IMS로 수송; **β-barrel 단백질은 SAM complex로 넘겨짐** |
...
| **OXA**   | Inner membrane | 미토콘드리아 genome 암호화 단백질, 혹은 TIM23에서 matrix로 넘어온 단백질을 innermembrane으로 삽입; matrix 쪽에서 접근              |
...
1. Precursor 단백질이 cytosol에서 **N-terminal signal sequence**와 함께 합성
	- signal sequence는 [[Sorting Signal#==Signal Sequences의 예시==|양전하 아미노산과 소수성 아미노산이 교대로 배치된]] [[Amphiphilic molecule|amphiphilic]]  [[α-helical Conformation in Membrane Proteins|α-helix]] 이다
...
| **(B) TOM → TIM22**           | Internal signal sequences가 TOM을 loop 형태로 통과; IMS chaperone이 안내 → TIM22가 multipass 단백질 삽입        |
| **(C) OXA (미토콘드리아 genome)**   | 미토콘드리아 ribosome이 matrix에서 합성 → OXA 복합체가 직접 inner membrane에 삽입                                   |
| **(D) TOM → matrix → OXA**    | N-terminal signal → matrix로 완전 수입 → MPP 절단 → 노출된 **두 번째 소수성 signal**이 OXA 경로로 inner membrane 삽입 |
...
- 포유류 세포 NPC: **3,000–4,000개**; 초당 최대 **1,000개 거대분자** 양방향 수송. Transcription factors도 포함.
...
- 능동 수송은 signal과 energy dependent하다.
- 결국 size와 signal에 의해 transport될지 말지가 결정된다.
- 핵공은 FG-repeat proteins에 의해 조절되는 선택적 게이트이다.
...
![[Cell Biology/출처/md converted/Chapter 12/_page_55_Figure_2.jpeg]]
...
==Calcineurin은 phosphatase 역할과 NES masking역할을 둘 다 한다.==
...
2. ==lamina가 분리된 염색체 주변에서 다시 형성됨==
	1. **RanGEF**가 chromatin에 결합 유지 → 염색체 주변 **Ran-GTP cloud** 형성
	2. Ran-GTP가 위치 표지 → 염색체 표면에 NPC 단백질 모집 및 새 NPC 조립
	3. 탈인산화된 lamin이 chromatin에 재결합 → inner nuclear membrane 단백질 모집
3. ==ER 막이 염색질을 감싸며 측면 융합==
4. ==lamin receptor를 가진 막 조각이 lamin 네트워크에 결합==
5. ==Membrane fusion==
6.  → 봉합된 nuclear envelope 재형성 (**engulfment**)
