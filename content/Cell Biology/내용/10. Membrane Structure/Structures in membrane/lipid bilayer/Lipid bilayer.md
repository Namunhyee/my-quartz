# Lipid Bilayer

Lipid bilayer는 모든 세포막의 기본 구조를 제공한다. 이는 전자현미경으로 쉽게 관찰되며, bilayer 구조는 전적으로 lipid molecule의 특별한 특성에 기인한다. Lipid molecule은 단순한 인공 조건에서도 자발적으로 bilayer로 조립된다.
## <mark style="background:#fff88f">전체 면적 대비 각 소기관에서의 면적 비율</mark>
- [[Endoplasmic Reticulum]] : 50~60%
- [[Mitochondria_MOC|Mitochondria]] : 20~30%
- [[Golgi apparatus_MOC|Golgi apparatus]] : 7%
- Plasma membrane : 2~5%

## <mark style="background:#fff88f">Protein Lipid Mass Ratio</mark>
<mark style="background:#fff88f">1:1</mark>

## ==Single Membrane vs Dounle Membrane==
### Lipid Bilayer (단일 이중층)
- 인지질(phospholipid) 두 겹으로 이루어진 막의 기본 구조 단위
- 각 층: 친수성 머리(hydrophilic head)가 바깥, 소수성 꼬리(hydrophobic tail)가 서로 맞닿음
- 역할: 세포막과 세포소기관 막의 **투과 장벽** 형성
- 해당 구조: 단일막 소기관 (ER, Golgi, Lysosome 등), 세포막(plasma membrane)
### Double Membrane (이중막)
- 두 겹의 **개별적인** lipid bilayer로 이루어진 구조 (bilayer × 2)
- 외막(outer membrane): 독립된 lipid bilayer
- 내막(inner membrane): 또 다른 독립된 lipid bilayer
- 그 사이: 막간 공간(intermembrane space)
- 외막과 내막이 각각 고유한 단백질과 기능을 가짐
- 내부 공생설(endosymbiotic origin)과 관련

| 특징  | Lipid Bilayer (단일) | Double Membrane           |
| --- | ------------------ | ------------------------- |
| 막 수 | 1개의 bilayer        | 2개의 독립된 bilayer           |
| 구조  | 단순                 | 내막/외막 + 막간 공간             |
| 예시  | ER, Golgi, 세포막     | 핵, 미토콘드리아, 엽록체            |
| 기원  | ER 유래              | 내부 공생설 또는 핵막 invagination |


## 구조적 특성
Lipid bilayer는 약 5 nm 두께의 연속적인 이중층으로 lipid molecule이 배열된 구조이다. 이 bilayer는 membrane의 기본적인 fluid structure를 제공하며 대부분의 water-soluble molecule의 통과에 대해 본질적으로 불투과성 장벽으로 작용한다.

### 분자 배열
![[Cell Biology/출처/md converted/Chapter 10/_page_5_Figure_7.jpeg]]

Lipid bilayer는 두 개의 monolayer (또는 leaflet)로 구성되며:
- 각 monolayer에서 lipid molecule의 hydrophobic tail은 안쪽을 향함
- Hydrophilic head group은 바깥쪽 수용액을 향함
- 두 monolayer의 hydrophobic tail이 서로 마주보며 bilayer의 core를 형성

## 형성 메커니즘
Lipid bilayer의 형성은 [[Amphiphilic molecule]]의 특성에서 비롯된다:

### 1. 자발적 조립 (Spontaneous Assembly)
[[Phospholipid]] molecule이 수용액에 노출되면:
1. Hydrophobic tail의 물 노출을 최소화하려는 경향
2. Hydrophilic head의 물 노출을 최대화하려는 경향
3. 이 두 경향이 특정 packing arrangement로 이어짐

### 2. 구조 결정 요인

Molecule의 모양이 최종 구조를 결정한다:

![[Cell Biology/출처/md converted/Chapter 10/_page_4_Figure_6.jpeg]]

- **Cone-shaped molecule**: [[Micelle]] 형성 (tail이 안쪽)
- **Cylinder-shaped molecule** (phospholipid 같은): Bilayer 형성
  - Hydrophobic tail이 두 hydrophilic head group 사이에 sandwiched
  - Double-layered sheet 구조

### 3. Self-sealing 특성

![[Cell Biology/출처/md converted/Chapter 10/_page_5_Figure_8.jpeg]]

Bilayer를 bilayer로 만드는 동일한 힘이 self-sealing property도 제공한다:

1. Bilayer에 small tear 발생
2. 물에 노출된 free edge 생성 - energetically unfavorable
3. Lipid가 자발적으로 재배열하여 free edge 제거
4. **중요한 결과**: Free edge를 피하는 유일한 방법은 자신을 닫아 sealed compartment 형성[^기출_중간_2021_11]

이 remarkable behavior는 살아있는 세포의 창조에 근본적이며, phospholipid molecule의 모양과 [[Amphiphilic molecule]] 특성에서 직접 나온다.

## Fluid 특성

![[Cell Biology/출처/md converted/Chapter 10/_page_6_Figure_1.jpeg]]

Lipid bilayer는 2차원 fluid이다:

### Lipid 분자의 운동

**Lateral diffusion (측면 확산):**
- 개별 lipid molecule이 bilayer plane 내에서 자유롭게 확산
- 이웃 molecule과 빠르게 위치 교환 (~10⁷ times/sec)
- Diffusion coefficient (D): ~10⁻⁸ cm²/sec
- 평균 lipid molecule이 큰 bacterial cell (~2 μm)의 길이를 약 1초에 확산

**Rotational diffusion:**
- Lipid molecule이 자신의 긴 축을 중심으로 빠르게 회전
- Flexible hydrocarbon chain 보유

**Flip-flop (매우 드묾):**
- 한 monolayer에서 다른 monolayer로의 이동
- 시간 척도: 수 시간
- 느린 이유: Hydrophilic head group이 일시적으로 bilayer의 hydrophobic core를 통과해야 함 - energetically disfavored
- 예외: [[Cholesterol]]은 빠르게 flip-flop (단일 hydroxyl group만 accommodating)

### Phospholipid Translocator

생물학적 membrane에서 문제가 발생한다:
- Phospholipid molecule은 주로 [[Lipid bilayer synthesis|ER membrane의 cytosolic monolayer에서만 제조됨]]
- 신규 분자가 noncytosolic monolayer로 빠르게 이동하지 못하면 membrane이 비대칭적으로 팽창

**해결책**: [[Flippase]]와 [[Scramblase]]
- Phospholipid translocator로 불리는 특별한 membrane protein class
- 한 monolayer에서 다른 monolayer로 phospholipid의 빠른 flip-flop 촉매

## Membrane으로서의 기능

Lipid bilayer는 membrane의 여러 필수 특성을 제공한다:

1. ==**투과 장벽**: 대부분의 water-soluble molecule에 대해 불투과성==
2. ==**유동성**: [[Fluidity of a lipid bilayer]]는 많은 membrane 기능에 중요==
3. **자가 봉합**: 손상 시 자발적으로 복구
4. ==**Compartmentalization**: Sealed compartment 형성으로 세포 경계 확립==

## Liposome

![[Cell Biology/출처/md converted/Chapter 10/_page_5_Figure_7.jpeg]]

Synthetic lipid bilayer는 liposome으로 만들어질 수 있다:
- 구형 vesicle 형태
- Model membrane으로 실험 연구에 사용
- 특히 incorporated membrane protein 연구에 유용

Liposome은 물 속에서 서로 자발적으로 융합하지 않는다:
- Polar lipid head group이 물 molecule과 ion을 결합
- 두 liposome의 bilayer가 충분히 가까이 오려면 이들이 제거되어야 함
- 이는 생물학적 membrane의 hydration shell을 더욱 크게 만듦
- 결과: 진핵세포의 많은 내부 membrane이 절연되어 통제되지 않은 융합 방지

모든 세포 membrane 융합 사건은:
- Tightly regulated tether에 의해 촉매됨
- [[SNARE|Fusion protein]]이 bilayer를 분리하는 물층을 강제로 제거

## 관련 개념
- [[Phospholipid]]
- [[Amphiphilic molecule]]
- [[Micelle]]
- [[Fluidity of a lipid bilayer]]
- [[Asymmetry of the lipid bilayer]]
- [[Major lipids in cell membrane]]
- [[Membrane protein]]

[^기출_중간_2021_11]: [[Cell Biology/기출 DB/중간고사/2021년/mid_2021_11번|2021 중간 11번]] — 인지질의 cylindrical shape으로 bilayer가 형성되고, free edge를 피해 자발적으로 sealed compartment를 형성하는 self-sealing 특성을 설명하는 서술형 문항. 복기 불완전.
